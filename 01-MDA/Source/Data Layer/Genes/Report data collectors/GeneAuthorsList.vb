...
Public Class GeneAuthorsList
    ...
    Public Overrides Function Apply(ByVal obj As LazyNode, ByVal ChildrenResults As Microsoft.VisualBasic.Collection) As Object
        ...
        Dim ChildSeries As LazyNodeCollection


        If obj.OriginationAuthor.Length > 0 Then
            Dim Authors As String() = obj.OriginationAuthor.Split(";")
            For Each Author As String In Authors
                Dim tuple As New AuthorItem(Author.Trim, obj.PhysLoc, obj.UnitDateInitial, obj.UnitDateFinal, obj.CompleteUnitId)
                Result.Add(tuple)
            Next
        End If

        For Each ChildResult As Collection In ChildrenResults
            For Each AuthorItem As AuthorItem In ChildResult
                Result.Add(AuthorItem)
            Next
        Next
